...
describe('modules/move - Move', () => {
    ...
    describe('Provider', () => {
        ...
        it('should throw an error if no shapes defined', () => {
            expect(() => move.elements()).toThrowError();
            expect(() => move.elements(null)).toThrowError();
            expect(() => move.elements(false)).toThrowError();
        });
    });
    ...
    describe('Behavior', () => {
        ...
        it('should move horizontal', () => {
            eventBus.fire('move.elements.by', {
                element: shape_1, dx: 5, dy: 0, x: 10, y: 20,
            });

            expect(shape_1.x).toEqual(105);
            expect(shape_1.y).toEqual(200);
        });

        it('should move vertical', () => {
            eventBus.fire('move.elements.by', {
                element: shape_1, dx: 0, dy: 5, x: 10, y: 20,
            });

            expect(shape_1.x).toEqual(100);
            expect(shape_1.y).toEqual(205);
        });
