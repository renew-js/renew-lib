...
export class FontStyleCommand extends Command {
    ...
    execute (attribute) {
        ...
        this.state.removed.forEach((shape) => {
            if (shape[0].type==='label') {
                const oldAtt = shape[0].options.style['font-style'];
                const element = shape[0];
                const newatt = attribute;
                if ( newatt === oldAtt) {
                    element.options.style['font-style'] = 'normal';

                } else {
                    element.options.style['font-style'] = attribute;
    ...
    revert (context) {
        ...
        this.state.removed.forEach((obj, index) => {
            const element = obj[0];
            element.options.style['font-style'] = obj[1];
            const newShape = element;
            this.canvas.addShape(newShape);
