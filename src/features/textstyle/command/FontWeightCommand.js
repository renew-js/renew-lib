...
export class FontWeightCommand extends Command {
    ...
    execute (attribute) {
        ...
        this.state.removed.forEach((shape) => {
            if (shape[0].type==='label') {
                const oldAtt = shape[0].options.style['font-weight'];
                const element = shape[0];
                const newatt = attribute;
                if ( newatt === oldAtt) {
                    element.options.style['font-weight'] = 'normal';

                } else {
                    element.options.style['font-weight'] = attribute;
    ...
    revert (context) {
        ...
        this.state.removed.forEach((obj, index) => {
            const element = obj[0];
            element.options.style['font-weight'] = obj[1];
            const newShape = element;
            this.canvas.addShape(newShape);
